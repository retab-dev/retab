...
public final class ParsesApi {
  private final RetabClient client;

  public ParsesApi(RetabClient client) {
    this.client = client;
  }
  ...
  public List<Parse> list(
    ...
    return client
        .getObjectMapper()
        .readValue(data.traverse(client.getObjectMapper()), new TypeReference<List<Parse>>() {});
  }

  public Parse create(ParseRequest request) throws IOException, InterruptedException {
    return create(
  ...
  public Object delete(String parseId) throws IOException, InterruptedException {
    String path = "/v1/parses/" + encodePathSegment(parseId);
    StringBuilder query = new StringBuilder();
    URI uri = URI.create(client.getBaseUrl() + path + (query.length() == 0 ? "" : "?" + query));
    HttpRequest.BodyPublisher publisher = HttpRequest.BodyPublishers.noBody();
