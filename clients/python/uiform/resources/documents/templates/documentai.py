...
class DocumentAIs(SyncAPIResource, BaseDocumentAIMixin):
    ...
    def parse(
        ...
        return maybe_parse_to_pydantic(request.model_dump(), DocumentExtractResponse.model_validate(response))
    ...
    def stream(
        ...
        for chunk_json in self._client._request_stream("POST", "/v1/documents/extractions", data=request.model_dump(), idempotency_key=idempotency_key):
            if not chunk_json:
                continue
            yield maybe_parse_to_pydantic(request.model_dump(), DocumentExtractResponse.model_validate(chunk_json), allow_partial=True)
        yield maybe_parse_to_pydantic(request.model_dump(), DocumentExtractResponse.model_validate(chunk_json))
