...
class WorkflowBlockExecutionsMixin:
    def prepare_list(
        self, run_id: str, block_id: str, before: str | None = None, after: str | None = None, limit: int | None = 20, order: PaginationOrder | None = "desc", **extra_params: Any
    ) -> PreparedRequest:
    ...
    def prepare_create(
        ...
        if extra_params:
            params.update(extra_params)
        params = {k: v for k, v in params.items() if v is not None}
        payload = CreateBlockExecutionRequest(run_id=run_id, block_id=block_id, step_id=step_id, n_consensus=n_consensus, check_eligibility=check_eligibility)
...
class WorkflowBlockExecutions(SyncAPIResource, WorkflowBlockExecutionsMixin):
    ...
    def list(
        ...
        """List Block Executions List recent block executions for one workflow run block. Cursor pagination matches the conventions used by ``GET /v1/extractions`` — pass ``after`` from the previous page's ``list_metadata.after`` to advance, ``before`` to step backwards, and ``order`` to flip the sort direction. ``run_id`` + ``block_id`` are required scope filters; without them this endpoint would expose…"""
        request = self.prepare_list(run_id=run_id, block_id=block_id, before=before, after=after, limit=limit, order=order, **extra_params)
    ...
    def create(
        ...
        """Create Block Execution Create a block execution for ``block_id`` against the current draft."""
        request = self.prepare_create(run_id=run_id, block_id=block_id, step_id=step_id, n_consensus=n_consensus, check_eligibility=check_eligibility, **extra_params)
