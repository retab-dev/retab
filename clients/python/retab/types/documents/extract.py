...
class DocumentExtractRequest(RetabBaseModel):
    ...
    n_consensus: int = Field(default=1, description="Number of consensus models to use for extraction.")
    # Regular fields
    ...
    chunking_keys: Optional[dict[str, str]] = Field(
        default=None, description="If set, keys to be used for the extraction of long lists of data using Parallel OCR", examples=[{"properties": "ID", "products": "identity.id"}]
    )
    ...
    additional_messages: Optional[list[ChatCompletionRetabMessage]] = Field(
        default=None, description="Additional chat messages to append after the document content messages. Useful for providing extra context or instructions."
    )
