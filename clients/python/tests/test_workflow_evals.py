...
"""Smoke tests for `client.workflows.evals.*` — wires + URL/method/body shape.
...
from retab.resources.workflows.evals import (
    AsyncWorkflowEvals,
    WorkflowEvals,
)
...
# ---------------------------------------------------------------------------
# Wiring — `client.workflows.evals` and `evals.runs` are present
# ---------------------------------------------------------------------------


def test_workflows_client_exposes_evals_subresource() -> None:
    """The `evals` sub-resource MUST be reachable from `client.workflows`.
    Without this wire, the docs example `client.workflows.evals.create(...)`
    raises `AttributeError` at import time for callers.
    """
    client = MagicMock()
    workflows = Workflows(client=client)
    assert isinstance(workflows.evals, WorkflowEvals)


def test_async_workflows_client_exposes_evals_subresource() -> None:
    client = MagicMock()
    workflows = AsyncWorkflows(client=client)
    assert isinstance(workflows.evals, AsyncWorkflowEvals)


def test_evals_resource_exposes_runs_subresource() -> None:
    """`evals.runs.list(...)` and `evals.runs.get(...)` are the only way
    to read run records from the SDK — the runs sub-resource MUST be
    initialized when WorkflowEvals is constructed.
    """
    client = MagicMock()
    evals = WorkflowEvals(client=client)
    assert evals.runs is not None
    # And the same on the async side.
    async_evals = AsyncWorkflowEvals(client=client)
    assert async_evals.runs is not None
...
def test_create_posts_to_evals_route_with_full_body() -> None:
    client = MagicMock()
    client._prepared_request.return_value = _TEST_RESPONSE

    test = Workflows(client=client).evals.create(
        workflow_id="wf_abc123",
        target={"type": "block", "block_id": "block_extract"},
        source={"type": "manual", "handle_inputs": {}},
        assertion={
    ...
    assert request.url == "/v1/workflows/evals"
...
def test_create_with_run_step_source_serializes_step_id() -> None:
    ...
    Workflows(client=client).evals.create(
        workflow_id="wf_abc123",
        target={"type": "block", "block_id": "block_extract"},
        source={
            "type": "run_step",
...
def test_get_uses_eval_detail_route() -> None:
    client = MagicMock()
    client._prepared_request.return_value = _TEST_RESPONSE

    Workflows(client=client).evals.get("wfnodeeval_abc")

    request = client._prepared_request.call_args.args[0]
    assert request.method == "GET"
    assert request.url == "/v1/workflows/evals/wfnodeeval_abc"


def test_list_uses_evals_route_with_filter() -> None:
    client = MagicMock()
    ...
    result = Workflows(client=client).evals.list(
        workflow_id="wf_abc123",
...
def test_list_omits_target_block_id_when_unset() -> None:
    ...
    Workflows(client=client).evals.list(workflow_id="wf_abc123")

    request = client._prepared_request.call_args.args[0]
    assert "target_block_id" not in request.params


def test_delete_uses_eval_detail_route() -> None:
    client = MagicMock()
    client._prepared_request.return_value = None

    Workflows(client=client).evals.delete("wfnodeeval_abc")
...
def test_update_only_includes_fields_the_caller_passed() -> None:
    ...
    Workflows(client=client).evals.update(
...
def eval_runs_create_with_eval_id_only() -> None:
    client = MagicMock()
    client._prepared_request.return_value = _run_response()

    response = Workflows(client=client).evals.runs.create(
        "wf_abc123",
        scope={"type": "single", "eval_id": "wfnodeeval_abc"},
    )

    request = client._prepared_request.call_args.args[0]
    assert request.method == "POST"
    assert request.url == "/v1/workflows/evals/runs"
    assert request.data == {
        "workflow_id": "wf_abc123",
        "scope": {"type": "single", "eval_id": "wfnodeeval_abc"},
    }
    assert response.lifecycle.status == "pending"
    assert response.id == "wfevalrun_q1z2"


def eval_runs_create_with_block_scope() -> None:
    client = MagicMock()
    client._prepared_request.return_value = _run_response()

    Workflows(client=client).evals.runs.create(
        "wf_abc123",
...
def eval_runs_create_no_args_runs_every_eval_in_workflow() -> None:
    """Only workflow_id in the body = run every eval in the workflow."""
    client = MagicMock()
    client._prepared_request.return_value = _run_response()

    Workflows(client=client).evals.runs.create("wf_abc123")
...
def test_workflow_eval_result_accepts_bare_string_verdict() -> None:
    ...
    from retab.types.workflows.evals import WorkflowEvalResult

    for verdict in ("passed", "failed", "blocked", None):
        payload = {**_RESULT_RESPONSE, "verdict": verdict}
        parsed = WorkflowEvalResult.model_validate(payload)
        assert parsed.verdict == verdict


def eval_runs_list_uses_canonical_runs_route() -> None:
    client = MagicMock()
    client._prepared_request.return_value = {
        "data": [],
        "list_metadata": {"before": None, "after": None},
    ...
    result = Workflows(client=client).evals.runs.list(
        workflow_id="wf_abc123",
        eval_id="wfnodeeval_abc",
        limit=10,
    )
...
def eval_runs_get_uses_run_id_first_route() -> None:
    client = MagicMock()
    client._prepared_request.return_value = _RUN_RESPONSE

    run = Workflows(client=client).evals.runs.get("wfevalrun_q1z2")

    request = client._prepared_request.call_args.args[0]
    assert request.method == "GET"
    assert request.url == "/v1/workflows/evals/runs/wfevalrun_q1z2"
    assert run.lifecycle.status == "completed"


def eval_runs_cancel_uses_run_id_first_route() -> None:
    client = MagicMock()
    client._prepared_request.return_value = _run_response(lifecycle={"status": "cancelled"})

    run = Workflows(client=client).evals.runs.cancel("wfevalrun_q1z2")

    request = client._prepared_request.call_args.args[0]
    assert request.method == "POST"
    assert request.url == "/v1/workflows/evals/runs/wfevalrun_q1z2/cancel"
    assert request.data is None
    assert run.lifecycle.status == "cancelled"


def test_workflow_eval_run_lifecycle_uses_test_status_values() -> None:
    from pydantic import ValidationError

    from retab.types.workflows import WorkflowEvalRun

    run = WorkflowEvalRun.model_validate(_run_response(lifecycle={"status": "error", "message": "boom"}))
    assert run.lifecycle.status == "error"
    assert run.lifecycle.message == "boom"

    with pytest.raises(ValidationError):
        WorkflowEvalRun.model_validate(_run_response(lifecycle={"status": "awaiting_review"}))


def test_workflow_eval_result_matches_run_record_shape() -> None:
    from retab.types.workflows import WorkflowEvalResult

    result = WorkflowEvalResult.model_validate(
        {
            **_RESULT_RESPONSE,
            "workflow_eval_run_id": None,
            "lifecycle": None,
            "timing": None,
        }
    )
    ...
    assert result.workflow_eval_run_id is None
    assert result.lifecycle is None
    assert result.timing is None


def test_workflow_eval_drift_enums_match_openapi() -> None:
    from pydantic import ValidationError

    from retab.types.workflows import WorkflowEval

    valid = WorkflowEval.model_validate(
        {
            **_TEST_RESPONSE,
    ...
    with pytest.raises(ValidationError):
        WorkflowEval.model_validate({**_TEST_RESPONSE, "schema_drift": "fresh"})

    with pytest.raises(ValidationError):
        WorkflowEval.model_validate({**_TEST_RESPONSE, "assertion_drift_status": "unknown"})


def eval_runs_results_list_uses_run_id_first_results_route() -> None:
    client = MagicMock()
    client._prepared_request.return_value = {
    ...
    result = Workflows(client=client).evals.results.list("wfevalrun_q1z2")

    request = client._prepared_request.call_args.args[0]
    assert request.method == "GET"
    assert request.url == "/v1/workflows/evals/results"
    assert request.params == {"run_id": "wfevalrun_q1z2", "limit": 20, "order": "desc"}
    assert result.data[0].eval_id == "wfnodeeval_abc"
...
def test_eval_results_get_uses_flat_result_id_route() -> None:
    client = MagicMock()
    client._prepared_request.return_value = _RESULT_RESPONSE

    result = Workflows(client=client).evals.results.get("wfresult_abc")

    request = client._prepared_request.call_args.args[0]
    assert request.method == "GET"
    assert request.url == "/v1/workflows/evals/results/wfresult_abc"
    assert result.id == "wfresult_abc"
    assert result.eval_id == "wfnodeeval_abc"


def test_evals_hard_cutover_removes_legacy_execute_and_scoped_run_aliases() -> None:
    client = MagicMock()
    evals = Workflows(client=client).evals

    assert not hasattr(evals, "execute")
    assert not hasattr(evals.runs, "get_execution")
    assert hasattr(evals, "results")
    assert hasattr(evals.results, "get")
    assert not callable(evals.results)
...
@pytest.mark.asyncio
async def test_async_create_posts_to_evals_route() -> None:
    client = MagicMock()
    client._prepared_request = AsyncMock(return_value=_TEST_RESPONSE)

    test = await AsyncWorkflows(client=client).evals.create(
        workflow_id="wf_abc123",
        target={"type": "block", "block_id": "block_extract"},
        source={"type": "manual", "handle_inputs": {}},
        assertion={
...
async def test_async_runs_get_returns_typed_record() -> None:
    ...
    run = await AsyncWorkflows(client=client).evals.runs.get("wfevalrun_q1z2")

    assert run.id == "wfevalrun_q1z2"
    assert run.lifecycle.status == "completed"
...
def test_workflow_eval_response_parses_legacy_doc_with_null_assertion() -> None:
    """Pre-rewrite evals in storage may have ``assertion=None``. The SDK's
    `WorkflowEval` model has `assertion: AssertionSpec | None = None`
    ...
    test = WorkflowEval.model_validate(legacy_payload)
    assert test.assertion is None


@pytest.mark.asyncio
async def test_async_runs_list_uses_eval_runs_route() -> None:
    """Sync coverage exists at `eval_runs_list_uses_eval_runs_route`; pin
    ...
    await AsyncWorkflows(client=client).evals.runs.list(
        workflow_id="wf_abc123",
        eval_id="wfnodeeval_abc",
        limit=10,
    )
...
def test_workflow_evals_do_not_expose_wait_for_completion() -> None:
    client = MagicMock()

    assert not hasattr(Workflows(client=client).evals, "wait_for_completion")
    assert not hasattr(AsyncWorkflows(client=client).evals, "wait_for_completion")
