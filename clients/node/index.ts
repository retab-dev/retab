...
class UiFormClientFetcher extends AbstractClient {
  ...
  async _fetch(params: {
    ...
    if (params.auth) {
      let bearerToken = this.options.auth && "bearer" in this.options.auth ? this.options.auth.bearer : process.env["UIFORM_BEARER_TOKEN"];
      let masterKey = this.options.auth && "masterKey" in this.options.auth ? this.options.auth.masterKey : process.env["UIFORM_MASTER_KEY"];
      let apiKey = this.options.auth && "apiKey" in this.options.auth ? this.options.auth.apiKey : process.env["UIFORM_API_KEY"];
