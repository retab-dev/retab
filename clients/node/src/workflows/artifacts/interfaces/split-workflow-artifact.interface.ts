// This file is auto-generated by the Retab oagen emitter. Do not edit by hand.

import { z } from 'zod';
import type { FileRef, FileRefResponse } from '../../../extractions/interfaces/file-ref.interface.js';
import { ZFileRef, deserializeFileRef } from '../../../extractions/interfaces/file-ref.interface.js';
import type { RetabUsage, RetabUsageResponse } from '../../../extractions/interfaces/retab-usage.interface.js';
import { ZRetabUsage, deserializeRetabUsage } from '../../../extractions/interfaces/retab-usage.interface.js';
import type { SplitConsensus, SplitConsensusResponse } from '../../../splits/interfaces/split-consensus.interface.js';
import { ZSplitConsensus, deserializeSplitConsensus } from '../../../splits/interfaces/split-consensus.interface.js';
import type { SplitResult, SplitResultResponse } from '../../../splits/interfaces/split-result.interface.js';
import { ZSplitResult, deserializeSplitResult } from '../../../splits/interfaces/split-result.interface.js';
import type { Subdocument, SubdocumentResponse } from '../../../splits/interfaces/subdocument.interface.js';
import { ZSubdocument, deserializeSubdocument } from '../../../splits/interfaces/subdocument.interface.js';

export interface SplitWorkflowArtifact {
  /** Unique identifier of the split result */
  id: string;
  /** Information about the split file */
  file: FileRef;
  /** Model used for the split operation */
  model: string;
  /** Subdocuments used for the split operation */
  subdocuments: Subdocument[];
  /**
   * Number of consensus votes used
   * @default 1
   */
  nConsensus?: number;
  /** Free-form instructions supplied with the split request. */
  instructions?: string | null;
  /** The list of document splits with their assigned pages */
  output: SplitResult[];
  /** Consensus metadata for multi-vote split runs */
  consensus?: SplitConsensus | null;
  /** Usage information for the split operation */
  usage?: RetabUsage | null;
  /** When this artifact was written by the orchestrator. */
  createdAt: Date;
  /**
   * Artifact operation that determines the backing record type
   * @default "split"
   */
  operation: 'split';
}

export interface SplitWorkflowArtifactResponse {
  id: string;
  file: FileRefResponse;
  model: string;
  subdocuments: SubdocumentResponse[];
  n_consensus?: number;
  instructions?: string | null;
  output: SplitResultResponse[];
  consensus?: SplitConsensusResponse | null;
  usage?: RetabUsageResponse | null;
  created_at: string;
  operation: 'split';
}

export const ZSplitWorkflowArtifact = z.object({
  "id": z.string(),
  "file": ZFileRef,
  "model": z.string(),
  "subdocuments": ZSubdocument.array(),
  "nConsensus": z.number().int().optional(),
  "instructions": z.string().nullable().optional(),
  "output": ZSplitResult.array(),
  "consensus": ZSplitConsensus.nullable().optional(),
  "usage": ZRetabUsage.nullable().optional(),
  "createdAt": z.coerce.date(),
  "operation": z.literal('split'),
}) as z.ZodType<SplitWorkflowArtifact>;

export function deserializeSplitWorkflowArtifact(wire: SplitWorkflowArtifactResponse): SplitWorkflowArtifact {
  return {
    id: wire["id"],
    file: deserializeFileRef(wire["file"]),
    model: wire["model"],
    subdocuments: wire["subdocuments"].map((__i) => deserializeSubdocument(__i)),
    nConsensus: wire["n_consensus"],
    instructions: wire["instructions"],
    output: wire["output"].map((__i) => deserializeSplitResult(__i)),
    consensus: wire["consensus"] == null ? (wire["consensus"] as undefined) : (wire["consensus"] == null ? wire["consensus"] : deserializeSplitConsensus(wire["consensus"])),
    usage: wire["usage"] == null ? (wire["usage"] as undefined) : (wire["usage"] == null ? wire["usage"] : deserializeRetabUsage(wire["usage"])),
    createdAt: new Date(wire["created_at"]),
    operation: wire["operation"],
  };
}
