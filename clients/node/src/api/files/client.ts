...
async function fileBodyForPath(filePath: string): Promise<Blob | fs.ReadStream> {
  const bunFile = (
    globalThis as typeof globalThis & { Bun?: { file?: (filePath: string) => Blob } }
  ).Bun?.file?.(filePath);
  if (bunFile) {
    return bunFile;
  }
  ...
  async upload(mimeData: MIMEDataInput, options?: RequestOptions): Promise<UploadFileResponse> {
    if (await isLocalFilePath(mimeData)) {
      ...
      if (!uploadResponse.ok) {
        throw new Error(
          `Direct file upload failed: ${uploadResponse.status} ${await uploadResponse.text()}`
        );
      }
