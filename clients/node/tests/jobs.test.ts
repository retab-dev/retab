...
describe('Jobs API', () => {
  ...
  describe('endpoint completion', () => {
    test(
      'extract — job completes and returns stored extraction',
      async () => {
    ...
    test(
      'split — job completes and returns stored split',
      async () => {
        const job = await client.jobs.create({
          endpoint: '/v1/splits',
          request: {
            ...
            subdocuments: [
              {
                name: 'invoice_header',
                description: 'The header section with invoice number and date',
              },
              {
                name: 'invoice_details',
                description: 'The details section with amount and customer',
              },
            ],
            model: MODEL,
          },
        ...
        expect(completed.response!.body).toHaveProperty('id');
        expect(completed.response!.body).toHaveProperty('output');
        expect(Array.isArray(completed.response!.body.output)).toBe(true);
      },
      { timeout: TEST_TIMEOUT }
    );
