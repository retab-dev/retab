...
function collectNodeResourcePaths(target: Record<string, unknown>, pathPrefix = "", seen = new Set<unknown>()): string[] {
    if (seen.has(target)) {
        return [];
    }
    seen.add(target);
...
function findClassDefinition(source: string, predicate: (name: string, bases: string[]) => boolean): PythonClassDefinition {
    ...
    for (let index = 0; index < lines.length; index++) {
        ...
        while (endIndex < lines.length && !/^class\s+[A-Za-z_][A-Za-z0-9_]*(?:\(|:)/.test(lines[endIndex] ?? "")) {
            endIndex++;
        }
        return {
            name,
...
function listNestedPythonResources(initBlock: string): Array<{ resourceName: string; className: string }> {
    const seen = new Set<string>();
    const nestedResources: Array<{ resourceName: string; className: string }> = [];

    for (const match of initBlock.matchAll(/^ {8}self\.([a-z_][a-z0-9_]*)\s*=\s*([A-Za-z_][A-Za-z0-9_.]*)\(/gm)) {
...
function buildPythonImportMap(source: string, currentFile: string): Map<string, string> {
    ...
    for (const match of source.matchAll(/^from\s+([.\w]+)\s+import\s+(.+)$/gm)) {
        ...
        for (const rawImportedName of importedNames.split(",")) {
            const importedName = rawImportedName.trim().split(/\s+as\s+/)[0]?.trim();
            if (!importedName) {
                continue;
            }
...
function collectPythonClassMethods(filePath: string, className: string, seen = new Set<string>()): string[] {
...
function buildPythonResourceSurfaceFromClass(
    ...
    const classDefinition = findClassDefinition(source, (name, bases) => name === className && bases.includes("SyncAPIResource"));
    ...
    const children = listNestedPythonResources(initBlock).flatMap(({ resourceName: childResourceName, className: childClassName }) => {
        let childResourceFile: string | null = null;
        try {
            findClassDefinition(source, (name) => name === childClassName);
            childResourceFile = resourceFile;
        } catch {
            childResourceFile = importMap.get(childClassName) ?? resolvePythonResourceFile(resourceFile, childResourceName);
        }
        ...
        return buildPythonResourceSurfaceFromClass(childResourceFile, childClassName, `${resourcePath}.${childResourceName}`);
    });
...
function buildPythonResourceSurface(
    ...
    const source = readFileSync(resourceFile, "utf8");
    const classDefinition = findClassDefinition(source, (_name, bases) => bases.includes("SyncAPIResource"));
...
function buildExpectedPythonSurface(): PythonResourceSurface[] {
    ...
    return listNestedPythonResources(initBlock)
        .flatMap(({ resourceName, className }) => {
            const resourceFile = importMap.get(className) ?? resolvePythonResourceFile(null, resourceName);
            if (!resourceFile) {
                return [];
            }
    ...
    test("node client exposes the full python resource surface", () => {
        ...
        for (const { resource, methods } of expectedPythonSurface) {
            ...
            for (const method of methods) {
                expect(typeof (nodeResource as Record<string, unknown>)[method], `missing method ${resource}.${method}`).toBe("function");
            }
            ...
            expect(extraMethods, `unexpected python-style methods on ${resource}: ${extraMethods.join(", ")}`).toEqual([]);
        }
    });
