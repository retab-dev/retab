...
describe('Node SDK files upload request', () => {
  test('rejects signed bucket URLs for files.upload', async () => {
    const signedUrl =
      'https://storage.googleapis.com/uiform-eu-multiregion/test/invoice.pdf?X-Goog-Signature=abc';
  ...
  test('rejects HTTP URLs for files.upload', async () => {
    ...
    await expect(api.files.upload('http://example.com/invoice.pdf')).rejects.toThrow(
      'local file paths'
    );
    expect(recorder.request).toBeNull();
  });
  ...
  test('does not complete when direct storage upload fails', async () => {
    ...
    globalThis.fetch = (async () =>
      new Response('signature mismatch', { status: 403 })) as typeof fetch;

    try {
      await expect(api.files.upload(filePath)).rejects.toThrow(
        'Direct file upload failed: 403 signature mismatch'
      );
    } finally {
      globalThis.fetch = originalFetch;
    }
