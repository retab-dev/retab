...
impl<'a> WorkflowReviewVersionsApi<'a> {
    ...
    /// Returns an async [`futures_util::Stream`] that yields every `ReviewVersion`
    /// across all pages, advancing the `after` cursor under the hood.
    ///
    /// ```ignore
    /// use futures_util::TryStreamExt;
    /// let all: Vec<ReviewVersion> = self
    ...
    pub fn list_auto_paging(
        &self,
        params: ListParams,
    ) -> impl futures_util::Stream<Item = Result<ReviewVersion, Error>> + '_ {
