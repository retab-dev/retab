...
// help block (so `tests create --help` is enough to answer "what fields
// does --source-file expect?").
func TestWorkflowsTestsCreateLongDocumentsBothSourceShapes(t *testing.T) {
	long := workflowsTestsCreateCmd.Long
	if long == "" {
		t.Fatal("workflowsTestsCreateCmd.Long is empty")
	}
...
func TestWorkflowsTestsCreateSourceFileFlagHelpMentionsBothShapes(t *testing.T) {
	flag := workflowsTestsCreateCmd.Flag("source-file")
	if flag == nil {
		t.Fatal("workflows tests create missing --source-file flag")
	}
	usage := flag.Usage
