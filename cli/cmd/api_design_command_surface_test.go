...
func TestAPICommandSurfaceUsesCanonicalResourceActionNames(t *testing.T) {
	canonicalNames := map[string]bool{
		...
		"functions": true, "generate": true, "get": true, "grant": true, "grep": true, "hydrate": true, "inspect": true, "invitations": true,
...
func TestCoreAPIResourcesExposeExpectedCommandSurface(t *testing.T) {
	expectedChildren := map[string][]string{
		...
		"workflows blocks":              {"list", "get", "create", "update", "delete", "versions", "executions", "stats", "config", "api-calls", "functions"},
