...
func TestWorkflowsEdgesCreateRejectsAmbiguousStartAlias(t *testing.T) {
	...
	if !strings.Contains(stderr, "multiple start_document blocks") {
		t.Fatalf("stderr %q does not mention multiple start_document blocks", stderr)
	}
...
// TestWorkflowsEdgesCreateRewrapsServerEdgeIDConflictWhenIDIsAutoGenerated
// pins the user-facing error message when two CLI calls with the same
// (source, source_handle, target, target_handle) tuple collide on the
// deterministic auto-generated edge id. The server returns
// “409 — Edge ID already exists“ (because it only checks id uniqueness),
// which is misleading: the user never supplied an id. The CLI rewraps it
// into a message that names the real cause — the duplicate endpoint tuple.
func TestWorkflowsEdgesCreateRewrapsServerEdgeIDConflictWhenIDIsAutoGenerated(t *testing.T) {
	t.Setenv("RETAB_API_KEY", "test-key")
	t.Setenv("HOME", t.TempDir())
	...
	// The rewrap must mention both endpoints and explain that the auto-id
	// collided, instead of regurgitating the server's misleading "Edge ID
	...
	wants := []string{
		...
		"output-file-0",
		"input-file-document",
		"auto-generated edge id collided",
	}
	for _, want := range wants {
		if !strings.Contains(stderr, want) {
			t.Fatalf("stderr should mention %q in rewrapped error, got:\n%s", want, stderr)
		}
	}
	if strings.Contains(stderr, "Edge ID already exists") {
		t.Fatalf("CLI should not surface the misleading server message verbatim, got:\n%s", stderr)
	}
}

// TestWorkflowsEdgesCreatePassesThroughServerEdgeIDConflictWhenIDIsExplicit
// pins the inverse: when the user explicitly passed --id, a 409 from the
// server IS about a user-chosen id colliding, so the CLI should NOT rewrap
// the error; it should pass the server message through verbatim.
func TestWorkflowsEdgesCreatePassesThroughServerEdgeIDConflictWhenIDIsExplicit(t *testing.T) {
	t.Setenv("RETAB_API_KEY", "test-key")
	t.Setenv("HOME", t.TempDir())
	...
	resetWorkflowEdgesCreateFlags(t)
	if err := workflowsEdgesCreateCmd.Flags().Set("id", "edge_my_explicit_id"); err != nil {
		t.Fatal(err)
	}
	...
	if !strings.Contains(stderr, "Edge ID already exists") {
		t.Fatalf("CLI should pass through the server message when --id is explicit, got:\n%s", stderr)
	}
	if strings.Contains(stderr, "auto-generated edge id collided") {
		t.Fatalf("CLI should not claim auto-id collision when --id is explicit, got:\n%s", stderr)
	}
}
