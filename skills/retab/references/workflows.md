...
- Pass JSON into `start_json` nodes
...
run = client.workflows.runs.create(
    ...
    documents={
        "document-node-id": Path("invoice.pdf"),
    },
    json_inputs={
...
- `documents`: keys must match the workflow's document start node IDs
- `json_inputs`: keys must match the workflow's JSON start node IDs

## Outputs

- `status`: usually `pending`, `running`, `completed`, or `failed`
- `steps`: per-node execution details
- `final_outputs`: terminal outputs keyed by end node ID
